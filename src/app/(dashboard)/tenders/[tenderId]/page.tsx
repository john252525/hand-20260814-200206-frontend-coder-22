  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card><CardContent className="pt-6"><div className="text-sm text-neutral-500">Поставщиков</div><div className="text-2xl font-semibold flex items-center gap-2"><Users className="h-5 w-5" />{tender.suppliers_count || 0}</div></CardContent></Card>
      ...
      <Tabs defaultValue="description">
        ...
        <TabsContent value="description">
          <Card>
            ...
            <CardContent><p className="whitespace-pre-wrap">{tender.description || 'Нет описания'}</p></CardContent>
          </Card>
        </TabsContent>
            ...
            <CardContent>
              {tender.timeline?.length ? (
                <ol className="relative border-l border-neutral-200 ml-3">
                  {tender.timeline.map((item) => (
                    <li key={item.id} className="mb-6 ml-4">
                      <div className="absolute w-3 h-3 bg-primary-600 rounded-full -left-1.5 mt-1.5" />
                      <StatusBadge status={item.status} size="sm" />
                      <div className="text-sm text-neutral-500 mt-1">{item.details}</div>
                      <time className="text-xs text-neutral-400">{formatDateTime(item.timestamp)}</time>
                    </li>
                  ))}
                </ol>
              ...
              {tender.documents?.length ? (
                <div className="space-y-2">
                  {tender.documents.map((doc) => (
                    <div key={doc.id} className="flex items-center gap-2 p-2 bg-neutral-50 rounded">
                      <FileText className="h-4 w-4 text-neutral-400" />
                      <span className="text-sm">{doc.filename}</span>
                    </div>
                  ))}
                </div>
              ) : <p className="text-neutral-400">Нет документов</p>}
            </CardContent>
          </Card>
        </TabsContent>
